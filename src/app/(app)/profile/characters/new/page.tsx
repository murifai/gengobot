...
import { ChevronLeft, Volume2, AlertCircle } from 'lucide-react';
...
export default function NewCharacterPage() {
  ...
  const [existingCharactersCount, setExistingCharactersCount] = useState<number>(0);
  ...
  const isAtLimit =
    customCharactersLimit !== null && existingCharactersCount >= customCharactersLimit;

  // Fetch existing character count
  useEffect(() => {
    async function fetchCharacterCount() {
      try {
        ...
        if (response.ok) {
          ...
          const customCount = data.filter(
            (c: { isUserCreated: boolean }) => c.isUserCreated
          ).length;
          setExistingCharactersCount(customCount);
        }
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          {!loadingCharacters && (
            <Badge variant={isAtLimit ? 'warning' : 'secondary'} size="sm">
              {customCharactersLimit === null
                ? `${existingCharactersCount} karakter`
                : `${existingCharactersCount}/${customCharactersLimit}`}
            </Badge>
          )}
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        {/* Limit Warning */}
        {isAtLimit && !loadingCharacters && (
          <div className="mb-6 p-4 bg-warning/10 border border-warning/30 rounded-base">
            <div className="flex items-start gap-3">
              <AlertCircle className="h-5 w-5 text-warning shrink-0 mt-0.5" />
              <div>
                <p className="font-medium text-warning-foreground">Batas karakter tercapai</p>
                <p className="text-sm text-muted-foreground mt-1">
                  Anda telah mencapai batas {customCharactersLimit} karakter kustom untuk paket
                  Anda. Upgrade paket untuk membuat lebih banyak karakter.
                </p>
              </div>
            </div>
          </div>
